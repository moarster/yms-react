...
import { validateINN,validateRoutePoint } from '@/types/schemas/shipment-rfp'
        ...
        const validateRoute = () => {
            ...
            formData.route.forEach((point, index) => {
                ...
                pointErrors.forEach(error =>
                    errors.push(`Точка ${index + 1}: ${error}`)
                )
                ...
                if (!point.cargoList?.length) {
                    ...
                    point.cargoList.forEach((cargo, cargoIndex) => {
                        ...
                        if (cargo.cargoWeight < 0) {
                            errors.push(`Точка ${index + 1}, груз ${cargoIndex + 1}: Вес не может быть отрицательным`)
                        }
                        if (cargo.cargoVolume < 0) {
                            errors.push(`Точка ${index + 1}, груз ${cargoIndex + 1}: Объем не может быть отрицательным`)
                        }
                    })
                }
            })
