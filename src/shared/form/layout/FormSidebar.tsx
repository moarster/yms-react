...
export const FormSidebar: React.FC<FormSidebarProps> = ({ className = '', sections }) => {
                ...
                {section.items.map((item, itemIdx) => (
                  <div key={itemIdx}>
                    ...
                    <dd className="mt-1">
                      {item.type === 'status' ? (
                        <span
                          className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusColor(item.value)}`}
                        >
                          {item.value}
                        </span>
                      ) : (
