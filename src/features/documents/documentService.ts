...
import { DocumentStatus, DomainEntity, UserTask, UserTasks } from '@/types';
...
class DocumentService {
  async getShipmentRfps(
    searchParams?: ShipmentRfpSearchParams,
  ): Promise<PaginatedResponse<ShipmentRfp>> {
  ...
  async createShipmentRfp(data: ShipmentRfpData): Promise<ShipmentRfp> {
    const response = await apiClient.post<DomainEntity<ShipmentRfpData>>(
      '/domain/shipment-rfp/shipment-rfp',
      { data },
    );
    return response;
  }
  ...
  async updateShipmentRfp(id: string, data: ShipmentRfp): Promise<ShipmentRfp> {
    const response = await apiClient.put<ShipmentRfp>(
      `/domain/shipment-rfp/shipment-rfp/${id}`,
      data,
    );
    return response;
  }
